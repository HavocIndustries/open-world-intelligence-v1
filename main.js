...
import { BUILTIN_PLUGIN_MANIFEST } from './plugins/manifest.js';
...
async function init() {
  ...
  try {
    ...
    // Initialize the data-layer manager, then load every built-in layer as a
    // plugin from the declarative manifest. Discovery, dependency-ordered
    // registration, the attachDataManager back-references, per-plugin error
    // isolation, and sealing (finalizeRegistrations) are all handled by the
    // loader — the core no longer names individual layers. See
    // docs/PLUGIN-ARCHITECTURE.md.
    const dataManager = new DataLayerManager(viewer, {
      allowQaRegistration: import.meta.env.DEV,
    });
    const pluginLoadResult = await loadPlugins({
      viewer,
      dataManager,
      manifest: BUILTIN_PLUGIN_MANIFEST,
      stateRegistry: LAYER_STATE_REGISTRY,
      logger: console,
    });
    if (pluginLoadResult.failed.length) {
      console.warn(
        `[Plugins] ${pluginLoadResult.failed.length} plugin(s) failed to load: `
        + pluginLoadResult.failed.map((f) => `${f.id} (${describeError(f.error)})`).join(', '),
      );
    }
    ...
    console.error("Open World Intelligence initialization failed:", error);
